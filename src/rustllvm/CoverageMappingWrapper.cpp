...
//#include "CodeGenFunction.h"               // Also port from clang??
...
class WHAT_ABSTRACT_TYPE_FOR_STMT {
  // The use case is indexing the CounterMap using the Stmt pointer as an opaque key
public:
//    virtual ... = 0;
};

class WHAT_ABSTRACT_TYPE_FOR_DECL {
public:
//    virtual ... = 0;
};
...
//  CodeGenModule &CGM; // remove clang dependencies
  // CoverageSourceInfo &SourceInfo;  // Skipped ranges (maybe not needed for Rust, but needed for clang)
...
class CoverageMappingGen {
  ...
  llvm::DenseMap<const WHAT_ABSTRACT_TYPE_FOR_STMT *, unsigned> *CounterMap;
...
public:
  ...
  void emitCounterMapping(const WHAT_ABSTRACT_TYPE_FOR_DECL *D, llvm::raw_ostream &OS);
...
//                    /*VisitChildren=* /!Defaulted); // FIXME(richkadel): nested block comment
...
std::string normalizeFilename(StringRef Filename) {
  ...
  llvm::sys::path::remove_dots(Path, /*remove_dot_dot=* /true); // FIXME(richkadel): nested block comment
  return Path.str().str();
}
...
#define COVMAP_FUNC_RECORD(Type, LLVMType, Name, Init) LLVMType,
    ...
    FunctionRecordTy =
        llvm::StructType::get(Ctx, makeArrayRef(FunctionRecordTypes),
                              /*isPacked=* /true); // FIXME(richkadel): nested block comment
  }
